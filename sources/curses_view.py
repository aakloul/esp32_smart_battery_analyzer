...
class CursesView:
        ...
        * Arrow keys/PageUp/PageDown → scroll log view
        ...
        if ch in (ord("c"), ord("C")):
            ...
            if not target_str.isdigit():
                ...
                self.stdscr.addstr(
                    max_y - 3, 2, "Please enter a numeric value.", curses.A_BLINK
                )
                self.stdscr.refresh()
                curses.napms(1200)
                # draw_table(stdscr)
            ...
            if row:
                ...
                max_y, _ = self.stdscr.getmaxyx()
                self.stdscr.addstr(
                    max_y - 3, 2, f"No row with battery_label={target}", curses.A_BLINK
                )
                self.stdscr.refresh()
                curses.napms(1500)
    ...
    def _draw_table(self) -> None:
        ...
        for row_idx, battery_label in enumerate(sorted(self._rows.keys()), start=2):
            ...
            capacity = row.get("capacity", "-1") / 10.0

            cells = [
                str(row.get("device_uuid")).ljust(col_widths[0]),
                str(battery_label).rjust(col_widths[1]),
                f"{capacity:.1f}".rjust(col_widths[2]),
                str(row.get("resistance", "-1")).rjust(col_widths[2]),
                str(row.get("voltage", "-1")).rjust(col_widths[2]),
                str(row.get("discharge_current", "-1")).rjust(col_widths[2]),
