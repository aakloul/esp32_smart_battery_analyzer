...
(line_volt,) = ax_volt.plot([], [], color="#1f77b4", linewidth=2, label="Voltage (V)")
...
voltage_max_tag = ax_volt.text(
    ...
    ha="right",
    va="top",  # anchor at the top‑right corner
    fontsize=9,
    color="#1f77b4",
    backgroundcolor="w",
...
def animate(frame_idx):
    ...
    line_cap.set_data(
        data_df["uptime_s"], data_df["capacity_f"] * scale_factor + voltage_floor
    )

    # ----- 4️⃣ keep axes limits identical -------------------------------------
    ax_volt.set_xlim(data_df["uptime_s"].min(), data_df["uptime_s"].max())
...
# ----------------------------------------------------------------------
anim = FuncAnimation(fig, func=animate, interval=INTERVAL_MS, blit=False)
