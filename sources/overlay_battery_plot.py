...
class OverlayBatteryPlot:
    ...
    def draw(self, save_path=None):
        ...
        for bid in self.battery_ids:
            ...
            h_volt, = ax_volt.plot(df["uptime_s"], df["voltage_f"],
            ...
            if self.annotate:
                ...
                ax_volt.text(0.02, 0.5, annotation,
        ...
        all_uptime = pd.concat([df["uptime_s"] for df in self.data.values()
                                if not df.empty])
        all_voltage = pd.concat([df["voltage_f"] for df in self.data.values()
                                 if not df.empty])
...
def _parse_args():
    ...
    parser.add_argument(
        "-b", "--batteries", nargs="+", type=int, required=True,
