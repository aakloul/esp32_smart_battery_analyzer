...
class _OneBatteryPlot:
    ...
    def __init__(self, conn, battery_id, ax_volt, ax_cap,
                 max_points=2000, v_min=2.7, v_max=4.2,
                   u_min=0, u_max=15000, c_max=2500):
        self.conn = conn
        ...
        self.line_cap, = self.ax_cap.plot([], [], color="#ff8f0e",
...
class MultiBatteryLivePlot:
    ...
    """
    def __init__(self, db_path, battery_ids,
                 interval_ms=2000, max_points=2000):
        if not battery_ids:
        ...
        self.fig, self.ax_matrix = plt.subplots(
            self.rows, self.cols,
            figsize=(self.cols * 6, self.rows * 4),
            squeeze=False
        )
        ...
        for idx, bat_id in enumerate(battery_ids):
            ...
            plot = _OneBatteryPlot(conn=self.conn,
                                   battery_id=bat_id,
                                   ax_volt=ax_volt,
                                   ax_cap=ax_cap,
